...
- [x] 6.0 Web app del rider (mobile-first)
  ...
  - [x] 6.2 **Login** (email + password) i **Logout** (RF 27, RF 28)
  - [x] 6.3 **Recepció del vehicle**: on anar, documentació, hora; **buit si no hi ha assignació** (RF 29)
  - [x] 6.4 **Vehicle vinculat**: pantalla principal quan la moto està vinculada (RF 30)
  - [x] 6.5 **Deixar el vehicle**: botó de desvinculació (RF 31)
  - [x] 6.6 **Reportar incidència**: llistat del catàleg del seu client + **pujada de foto** (`lib/storage.ts`) (RF 32)
  - [x] 6.7 **Vehicles de substitució**: llistat del client+regió; **bloquejat si té vehicle vinculat** (RF 33)
  - [x] 6.8 **Auto-assignació** de substitució quan no té vehicle vinculat (RF 34)
  - [x] 6.9 **Obrir seient**: camp de **5 dígits** + Enviar, validat contra el codi generat al backoffice (RF 35)
  - [x] 6.10 Mostrar **"moto disponible per recollir"** via notificacions quan el ticket es tanca (RF 36)
  - [x] 6.11 **Mapa d'estacions de bateries**: bateries >80% per estació + deep link a **Google Maps** (`lib/maps.ts`) (RF 37) — llista+enllaç; tiles de mapa ajornats (proxy bloqueja hosts externs)
  - [x] 6.12 **Reporte per QR sense usuari** (`app/qr/[motoId]`): report sense login amb rate limiting bàsic (RF 38)
  - [x] 6.13 Tests dels fluxos crítics del rider (`lib/services/rider.test.ts`, 6/6 OK) + smoke test de rutes
